...
总体状态：进行中
当前阶段：模块 0 已完成；模块 1、模块 2、模块 3 的实现、模拟器构建和自动化验收已通过；模块 3 的摄像头重复进入缺陷已通过 M3-16 连续 10 轮真实 iPhone 复测，M3-02 同页连续拍摄、摄像头切换和中断恢复阻断项也已通过连续 10 轮真实 iPhone 复测；方向、镜像、蓝牙、相册、长录制及其他系统中断场景等其余真机验收仍待执行
...
| 3 | 摄像头与视频录制 | 进行中 | iPhone、iPad 模拟器 Debug、Generic iOS Device 无签名 Debug、Generic iOS Device 无签名 Release 构建通过 | 单元测试 162/162；UI 测试 21/21（含模块 1、2 全量回归）；模块 2 性能阈值继续通过 | M3-02、M3-16 通过；M3-01、M3-03 至 M3-15 待执行 | 同页连续拍摄、摄像头切换和中断恢复阻断项已解除；完成片段后显式执行 `.stopping → .finished → .ready`，摄像头切换只替换视频输入，中断结束后须用户明确重新准备 |
...
- 模块 3 整体仍为“进行中”：M3-01、M3-03 至 M3-15 的真实权限、4K、方向、镜像、焦点曝光、防抖、蓝牙音频、更多系统中断、低空间、相册、30 分钟录制、热量与音画同步等项目仍待执行。本轮未开始模块 4。
