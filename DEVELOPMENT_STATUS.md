...
总体状态：进行中
当前阶段：模块 0 已完成；模块 1、模块 2、模块 3 的实现、模拟器构建和自动化验收已通过；模块 3 的 M3-02 同页连续拍摄、M3-03 前后摄 4K/30 fps、M3-04 镜像与 UI/成片隔离、M3-05 iPhone 方向与起录方向锁定，以及 M3-16 摄像头重复进入均已通过真实 iPhone 验收；M3-06 点击对焦、曝光和真实设备锁定验收失败，修复前仍是阻断项；权限、防抖、蓝牙、相册、长录制、iPad 方向及其他系统中断场景等其余真机验收仍待执行
...
| 3 | 摄像头与视频录制 | 进行中 | iPhone、iPad 模拟器 Debug、Generic iOS Device 无签名 Debug、Generic iOS Device 无签名 Release 构建通过 | 单元测试 162/162；UI 测试 21/21（含模块 1、2 全量回归）；模块 2 性能阈值继续通过 | M3-02、M3-03、M3-04、M3-05、M3-16 通过；M3-06 失败；M3-01、M3-07 至 M3-15 待执行 | 前后摄 4K/30 fps 真机参数通过；点击对焦、曝光和真实设备锁定未生效，M3-06 待修复与复测；模块 3 仍有其余真机项目 |
...
- 模块 3 整体仍为“进行中”：M3-06 是当前明确的点击对焦、曝光与真实设备锁定阻断项；M3-01、M3-07 至 M3-15 的真实权限、防抖、蓝牙音频、更多系统中断、低空间、相册、30 分钟录制、热量与音画同步，以及 DEV-05 的 iPad 方向覆盖等项目仍待执行。本轮未开始模块 4。
