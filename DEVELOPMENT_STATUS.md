...
总体状态：进行中
当前阶段：模块 0 已完成；模块 1、模块 2、模块 3 的实现、模拟器构建和自动化验收已通过；模块 3 的 M3-01 摄像头与麦克风权限按需请求及拒绝恢复、M3-02 同页连续拍摄、M3-03 前后摄 4K/30 fps、M3-04 镜像与 UI/成片隔离、M3-05 iPhone 方向与起录方向锁定、M3-06 对焦曝光与镜头切换状态清理、M3-16 摄像头重复进入、M3-17 单播放器与照片保存反馈，以及 M3-18 实时计时与最终媒体时长一致性均已通过真实 iPhone 验收；防抖、蓝牙、长录制、iPad 方向及其他系统中断场景等其余真机验收仍待执行
...
| 3 | 摄像头与视频录制 | 进行中 | iPhone、iPad 模拟器 Debug、Generic iOS Device 无签名 Debug、Generic iOS Device 无签名 Release 构建通过 | 单元测试 212/212、UI 测试 25/25；模块 2 性能三轮 24/24 通过 | M3-01 至 M3-06、M3-16 至 M3-18 通过；M3-07 至 M3-15 待执行 | 录制请求与 AVFoundation 确认写入已拆分；实时计时从 `didStartRecordingTo` 的单调时钟起点推导，最终时长仍以封口媒体为准且保留 2 秒恢复分片；模块 3 仍有其他真机项目 |
...
- M3-01 至 M3-06 与 M3-16 至 M3-18 已通过，但真实防抖、Bluetooth 路由、更多系统中断、媒体服务重置、低空间封口、Photos、30 分钟热量/功耗/音画同步与异常文件可播放性仍需 M3-07 至 M3-15 验证，因此模块 3 状态保持“进行中”。
...
- 中断生命周期现按来源跟踪 Capture Session、AVAudioSession、App 前后台和存储监控。Capture Session 的开始/结束通知在安装观察者时捕获不可变的 session ID 与 service generation；ViewModel 的摄像和音频事件再校验页面 lifecycle generation，旧通知不能污染新页面。真实中断先进入 `.interrupted`；录制中必须等可恢复文件完成封口，且所有中断来源均结束后才进入 `.recoveryRequired`，由用户点击“重新准备摄像头”执行完整关闭与重新初始化，不自动开始录制，也不直接开放切换按钮。
- 仅 Debug 构建新增结构化诊断，记录单调时间、状态、lifecycle generation、截短 session token、摄像头位置、录制/封口/重配置标志、映射后及原始 Capture Session 中断 reason、开始或结束；AVAudioSession 另记录原始中断 type/options。日志不含稿件正文、媒体内容或完整本地路径，Release 不输出这些诊断。
...
- M3-03 真实设备验收通过（2026-07-30）：iPhone 16 / iOS 26.5.2，测试版本 `23e9d9875ba015e4664c5c7af2f06224f49dee7f`。前摄与后摄均显示“4K · 30 fps”并录制成功；输出为 MOV、3840×2160、HEVC、30 FPS、AAC 48,000 Hz、16:9；方向和比例正确，文件均可播放且有声音，全程无异常。
